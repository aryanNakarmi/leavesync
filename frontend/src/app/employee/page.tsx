...
export default function EmployeeDashboard() {
  ...
  const quickActions = [
    { title: "Apply for Leave", desc: "Submit a new leave request", icon: "add_circle", href: "/employee/apply", color: "bg-primary-fixed text-primary" },
    { title: "My Requests", desc: "View your leave history", icon: "pending_actions", href: "/employee/status", color: "bg-secondary-fixed text-secondary" },
    { title: "Calendar", desc: "View your schedule", icon: "calendar_month", href: "/employee/calendar", color: "bg-primary-fixed text-primary" },
  ];

  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-on-surface">Welcome back, {session?.user?.name?.split(" ")[0]} 👋</h1>
      ...
      <div className="bg-white rounded-xl border border-outline-variant shadow-sm">
        ...
        {loading ? (
          ...
          <div className="p-8 text-center">
            ...
            <button onClick={() => router.push("/employee/apply")} className="mt-3 text-sm text-primary font-medium hover:underline">Apply for your first leave</button>
          </div>
        ) : (
